...
vec2 watercolor_noise(vec2 uv, float time_seed, int num_iterations) {
    vec2 noise = vec2(0.0);
    for (int i = 1; i < num_iterations; i++) {
        float factor = 1.0 / float(i);
        float freq = float(i) * 2.5;

        float angle = freq * uv.x + freq * uv.y + time_seed;
        noise += vec2(sin(angle), cos(angle)) * factor;
    }
    return uv + noise * .1;
}
...
void main() {
    ...
    for (int i = 0; i < MAX_BUBBLES; i++) {
        ...
        // di: Signed distance to bubble edge. positive for outside, negative for inside
        ...
        vec3 outline_color = hsv2rgb(vec3(fract((mask_uv.x + mask_uv.y) * 0.15 + time_seed * .5), 0.6, 1.0), target_color);
        ...
        vec3 interior = hsv2rgb(vec3(fract((color_uv.x + color_uv.y) * 0.15 + time_seed), 0.4, 1.0), target_color);
        interior *= smoothstep(0.98, 1.0, sin(dot(mask_uv, vec2(1.2, 1.7))));
        float interior_strength = .5 *clamp(-di / (radius + 1e-6), 0.0, 1.0);
